...
use App\Models\Central\TenancyLanguageLine;
use App\Models\Central\Tenant;

class LanguageLineResource extends Resource
{
    use CanRegisterPanelNavigation;
    protected static ?string $model = TenancyLanguageLine::class;
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                Section::make('Client specific translations')->schema(static::getTenantsTranslations())
                    ->columns(1)
            ]);
    }
    ...
    public static function getColumns(): array
    {
        ...
        foreach (config('translation-manager.available_locales') as $locale) {
            ...
            $columns[] = IconColumn::make($localeCode)
                ...
                ->searchable(false)
                ->sortable(false)
                ->getStateUsing(function (TenancyLanguageLine $record) use ($localeCode) {
    ...
    public static function getTenantsTranslations(): array
    {
        $tenants = Tenant::all();
        $translations = [];
        foreach ($tenants as $tenant) {
            $translations[] = Section::make($tenant->name)
                ->schema([
                    Repeater::make('tenancy_translations')->schema([
                        Select::make('language')
                            ->prefixIcon('heroicon-o-language')
                            ->label(__('translation-manager::translations.translation-language'))
                            ->options(collect(config('translation-manager.available_locales'))->pluck('code', 'code')),

                        Textarea::make($tenant->id)
                            ->label(__('translation-manager::translations.translation-text')),
